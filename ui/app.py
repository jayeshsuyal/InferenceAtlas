"""Streamlit UI for LLM deployment recommendations."""
...
st.set_page_config(page_title="GPU Selector", layout="centered")

st.title("GPUSelector: LLM Deployment Optimizer")
st.caption("Choose the right GPU/platform for your workload with monthly cost estimates.")
...
with st.form("inputs"):
    ...
    pattern_label = st.selectbox("Traffic Pattern", ["Steady", "Business Hours", "Bursty"])
    latency_requirement_ms = st.number_input(
        "Latency requirement (ms, optional)",
        min_value=0.0,
        value=0.0,
        step=10.0,
        help="Set to 0 to ignore latency constraint.",
    )
